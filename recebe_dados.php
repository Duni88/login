...
if (isset($_POST['action'])) {
    ...
    if ($_POST['action'] == 'cadastro') {
        ...
        $senhaDoUsuario = verificar_entrada($_POST['emailUsuario']);
        ...
        // echo "<p>Data de criaçao: $dataCriado</p>";
        if($senhaCodificada != $senhaConfirmarCod){
            ...
            if($linha['nomeDoUsuario'] == $nomeDoUsuario){
                echo "<p class='texte-danger'> Usuario indisponivel </p>";
            }elseif ($linha['emailUsuario']== $emailUsuario){
                echo "<p class='texte-danger'> E-mail indisponivel </p>";
            }else{
                //usuario pode ser cadastrado no banco de dados
                $sql = $connect->prepare("INSERT into usuario (nomeDoUsuario, nomeCompleto,
                emailUsuario, senhaDoUsuario, dataCriada)values(?, ?, ?, ?, ?)");
                $sql->bind_param("sssss", $nomeDoUsuario,
                $nomeCompleto, $emailUsuario, $senhaCodificada, $dataCriado);
                if($sql->execute()){
